...
class SnapshotRestorer {
  ...
  Future<bool> restoreIfNeeded() async {
    ...
    await _db.transaction(() async {
      ...
      await _db.profileDao.upsertProfile(PlayerProfilesCompanion.insert(
      ...
      await _db.statsDao.overwriteFromSnapshot(PlayerStatsTableCompanion.insert(
      ...
      for (final entry
          in ((stats['enemy_kills'] as Map<String, dynamic>?) ?? const {})
              .entries) {
        await _db.statsDao
            .overwriteCounter('enemyKill', entry.key, _asInt(entry.value));
      }
      for (final entry
          in ((stats['upgrade_picks'] as Map<String, dynamic>?) ?? const {})
              .entries) {
        await _db.statsDao
            .overwriteCounter('upgradePick', entry.key, _asInt(entry.value));
      }
      ...
      for (final a in (snapshot['achievements'] as List? ?? const [])
        ...
        final unlockedAt =
            DateTime.tryParse(a['unlocked_at'] as String? ?? '')
                ...
                nowMs;
        await _db.achievementsDao.upsertState(AchievementStateRow(
      ...
      for (final c in (snapshot['challenge_results'] as List? ?? const [])
          .cast<Map<String, dynamic>>()) {
