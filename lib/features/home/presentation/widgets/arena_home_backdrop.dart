...
/// The home screen's cinematic backdrop — a window onto the arena: a neon
/// perspective floor grid scrolling toward the viewer, a couple of ricochet
/// tracer bullets bouncing off the edges (the core mechanic, alive), drifting
/// enemy silhouettes, rising embers, and a vignette. Richer than the shared
/// [AnimatedArenaBackground] (which stays light for the other screens).
class ArenaHomeBackdrop extends StatefulWidget {
  const ArenaHomeBackdrop({super.key, this.child});

  final Widget? child;

  @override
  State<ArenaHomeBackdrop> createState() => _ArenaHomeBackdropState();
}

class _ArenaHomeBackdropState extends State<ArenaHomeBackdrop>
  ...
  late final List<_Ember> _embers =
      List.generate(24, (i) => _Ember.seeded(i));
...
class _BackdropPainter extends CustomPainter {
  ...
  void _paintFloor(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < rows; i++) {
      ...
      final alpha = (0.22 * (1 - d)).clamp(0.0, 0.22);
      canvas.drawLine(
        Offset(0, y),
        Offset(size.width, y),
        Paint()
  ...
  void _paintSilhouettes(Canvas canvas, Size size) {
    for (final s in silhouettes) {
      ...
      final paint = Paint()
        ..color = AppColors.ink500.withValues(alpha: 0.22)
  ...
  void _paintTracers(Canvas canvas, Size size) {
    ...
    for (final tr in tracers) {
      ...
      final head = at(t);
      canvas.drawCircle(
          head, 6, Paint()..color = color.withValues(alpha: 0.25));
      canvas.drawCircle(head, 2.4,
          Paint()..color = AppColors.amber100.withValues(alpha: 0.9));
    }
  }
